...
void leer_parametros(struct poblaciones *poblacion){
    FILE *f1;
...
void inicializar(int tam_poblacion, struct persona_virus *personas, int tam_escenario){

    int i;
    
    personas = malloc(tam_poblacion*sizeof(struct persona_virus));

    calc_edad(tam_poblacion, personas);
    
    for(i=0; i<tam_poblacion; i++){
        personas[i].t_estado = 0; //Tiempo en el estado 0
        personas[i].estado = 0; //Todos sanos
        personas[i].pos[0] = calc_pos(tam_escenario, i); 		//Beti zenbaki berdina ez emateko semilla bezela i pasa
        personas[i].pos[1] = calc_pos(tam_escenario, i+1);
