...
void movimiento(struct persona_virus *personas, struct poblaciones *poblacion  ){
	
	int i;
	//Velocidad denentzat bera ez izateko.
	srand(time(0));
	for(i=0; i<poblacion->tam; i++){
		...
		if((personas[i].pos[0]+personas[i].vel[0]) < poblacion->tam_escenario || (personas[i].pos[0]+personas[i].vel[0]) > 0)
			personas[i].pos[0] += personas[i].vel[0];

		if((personas[i].pos[1]+personas[i].vel[1]) < poblacion->tam_escenario || (personas[i].pos[1]+personas[i].vel[1]) > 0)
			personas[i].pos[1] += personas[i].vel[1];


		//Hurrengo iterazioarako velocidad aldatu.
		personas[i].vel[0] = calc_vel(i);
		personas[i].vel[1] = calc_vel(i+24);
	}
}
		...
		if(personas[i].estado == 2 && personas[i].t_estado!=0){ 
			...
			for(int j=0; j<poblacion->tam; j++){
				for(int z=-poblacion->radio_cont; z<poblacion->radio_cont; z++){
					if(personas[i].pos[0] == personas[j].pos[0]+z && personas[j].estado == 0){
						for(int k=-poblacion->radio_cont; k<poblacion->radio_cont; k++){
							if(personas[i].pos[1] == personas[j].pos[1]+k && personas[j].estado == 0){
