...
void calc_edad(int numero_personas, struct persona_virus *personas){
...
int calc_pos(int tam_escenario, int j){
	srand(j);									//beti zenbaki bera ez emateko semilla bezela j pasa
...
//Return 1  EZ da HIL
int p_morir(int prob_morir){
	srand(time(0));
	//0 edo 1 zenbaki bat hemango du.
	int j = rand()%101;
	if(j < (prob_morir*100)) return 0;   // HIL EGIN DA
